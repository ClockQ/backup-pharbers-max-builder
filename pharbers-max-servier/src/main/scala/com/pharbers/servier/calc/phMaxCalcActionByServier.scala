...
object phMaxCalcActionByServier {
    def apply(args: pActionArgs = NULLArgs): pActionTrait = new phMaxCalcActionByServier(args)
}

class phMaxCalcActionByServier(override val defaultArgs: pActionArgs) extends pActionTrait {
    ...
    override def perform(prMap: pActionArgs): pActionArgs = {
        ...
        val panelDF = {
            prMap.asInstanceOf[MapArgs].get("panel_data").asInstanceOf[DFArgs].get
        ...
        val universeDF = {
            prMap.asInstanceOf[MapArgs].get("universe_data").asInstanceOf[DFArgs].get
                    ...
                    .distinct()    //施维雅样本医院对应有多个原始ID对应一个PHA ID的情况
        }
            ...
            joinData
                    ...
                    .withColumn("f_sales",
                        when($"IS_PANEL_HOSP" === 1, $"sumSales").otherwise(
                            when($"avg_Sales" <= 0.0 or $"avg_Units" < 0.0, 0.0)  //对于施维雅，就算units=0，也不需要改动sales和units
