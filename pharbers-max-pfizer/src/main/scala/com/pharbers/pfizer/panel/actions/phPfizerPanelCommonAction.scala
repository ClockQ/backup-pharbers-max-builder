package com.pharbers.pfizer.panel.actions
...
class phPfizerPanelCommonAction(override val defaultArgs: pActionArgs) extends pActionTrait {
    ...
    override def perform(args: pActionArgs): pActionArgs = {
        ...
        val cpa = {
            args.asInstanceOf[MapArgs].get("cpa").asInstanceOf[DFArgs].get
        ...
        val gycx = {
            args.asInstanceOf[MapArgs].get("gycx").asInstanceOf[DFArgs].get
                    .na.fill(value = "0", cols = Array("VALUE", "STANDARD_UNIT"))
                    .withColumn("PRODUCT_NAME", when(col("PRODUCT_NAME").isNull, col("MOLE_NAME"))
                            .otherwise(col("PRODUCT_NAME")))
                    .withColumn("MONTH", 'MONTH.cast(IntegerType))
        ...
        //通用名市场定义 =>表b0　
        val markets_match = {
            args.asInstanceOf[MapArgs].get("markets_match_file").asInstanceOf[DFArgs].get
...
//            .filter(col("Date") === ym)
        ...
        def fullCPAandGYCX = {
            ...
            import sparkDriver.ss.implicits._
            reduced_cpa.union(full_hosp).union(gycx.filter(s"YM like '$ym'").select(reduced_cpa.columns.head, reduced_cpa.columns.tail: _*)).withColumn("HOSP_ID", 'HOSP_ID.cast(LongType))
        }
        ...
        def trimPanel(filted_panel: DataFrame, min1_match: DataFrame): DataFrame = {
            ...
            temp.groupBy("ID", "Hosp_name", "Date", "Prod_Name", "Prod_CNAME", "HOSP_ID", "Strength", "DOI", "DOIE") //"HOSP_ID", "Strength" 没有重复
