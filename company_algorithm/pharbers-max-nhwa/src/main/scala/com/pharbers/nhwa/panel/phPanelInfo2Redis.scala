...
class phPanelInfo2Redis(override val defaultArgs: pActionArgs) extends pActionTrait with phLogTrait {
    ...
    override def perform(pr: pActionArgs): pActionArgs = {
        ...
        val condition = "Prod_Name like '%a%'"//getAllSubsidiary(company).map(x => s"Prod_Name like '%$x%'").mkString(" OR ") //获得所有子公司
        ...
        phLog(s"准备 -- 存储${mkt}市场的[panel]聚合数据到Redis中")
        ...
        phLog(s"开始 -- 存储${mkt}市场的[panel]聚合数据到Redis中")
...
//        rd.expire(not_panel_hosp_key, 60*60*24)
        ...
        phLog(s"完成 -- 存储${mkt}市场的[panel]聚合数据到Redis中")
