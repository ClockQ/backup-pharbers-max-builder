...
class phAstellasPanelConcretJob(override val defaultArgs: pActionArgs) extends pActionTrait {
    ...
    override def perform(prMap: pActionArgs): pActionArgs = {
        ...
        val cpa = {
            prMap.asInstanceOf[MapArgs].get("cpa").asInstanceOf[DFArgs].get
                    .filter(col("YM") === ym)
        ...
        val gycx = {
            prMap.asInstanceOf[MapArgs].get("gycx").asInstanceOf[DFArgs].get
                .filter(col("YM") === ym)
                .withColumn("PRODUCT_NAME", when(col("PRODUCT_NAME").isNull, col("MOLE_NAME"))
                        .otherwise(col("PRODUCT_NAME")))
                .withColumn("VALUE", when(col("STANDARD_UNIT") === "0", "0")
        ...
        val product_match_file = {
            prMap.asInstanceOf[MapArgs].get("product_match_file").asInstanceOf[DFArgs].get
        ...
        val markets_match = {
            prMap.asInstanceOf[MapArgs].get("markets_match_file").asInstanceOf[DFArgs].get
        }

        val hospital = {
            prMap.asInstanceOf[MapArgs].get("hospital_file").asInstanceOf[DFArgs].get
        ...
        val hosp_ID = {
            prMap.asInstanceOf[MapArgs].get("hosp_ID_file").asInstanceOf[DFArgs].get
        }
        ...
        val product_match = product_match_file
                .withColumn("MIN_PRODUCT_UNIT", regexp_replace(col("MIN_PRODUCT_UNIT"), "\\|", ""))
